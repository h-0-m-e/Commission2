...
fun limitChecker(
    ...
    if (cardType == "VkPay") {
        if (amountToSend + sentAtThisMonth > vkPayStopLimitAtMonth) {
            return false
        }
...
fun commissionCalculator(
    ...
    if (cardType == "Visa" || cardType == "MIR") {
        if ((amountToSend / 100) * visaAndMIRPercent > visaAndMIRMinCommission) {
            return ((amountToSend / 100) * visaAndMIRPercent).toInt()
        } else {
            return visaAndMIRMinCommission
        }
    } else {
